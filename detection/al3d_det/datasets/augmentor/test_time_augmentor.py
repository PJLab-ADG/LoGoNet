...
from al3d_utils.common_utils import rotate_points_along_z
...
class TestTimeAugmentor(object):
    ...
    def world_translation(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.world_translation, config=config)
        trans_list = config['TRANS_PARAM']
        data_dict_list = []
        for trans_val in trans_list:
            if trans_val == 0.: continue
            data_dict_trans = copy.deepcopy(data_dict)
            data_dict_trans['points'][:, 0] += trans_val
            data_dict_trans['points'][:, 1] += trans_val
            data_dict_trans['points'][:, 2] += trans_val
            # data_dict_trans['points'][:, :2] += trans_val
            key_name = 'tta_trans_%s' % str(trans_val)
            data_dict_list.append({key_name: data_dict_trans})
        return data_dict_list
