...
class DetFPNTemplate(nn.Module):
    ...
    def __init__(self, constructor, feat_extract_layer, num_classes, pretrained_path=None):
        ...
        # self.model.backbone.body.return_layers.update(return_layers)
    ...
    def forward(self, images):
        ...
        # Extract features
        result = OrderedDict()
        features = self.model.backbone.body(x)
        features = self.model.backbone.fpn(features)
        # import pdb; pdb.set_trace()
        for _layer in self.feat_extract_layer:
            result[_layer] = features["{}".format(self.feat_extract_layer.index(_layer))]
        
        result["logits"] = features["3"]
        return result
